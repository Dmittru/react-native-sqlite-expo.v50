...
export function createOrSyncTableHotels() {
    db.transaction((tx) => {
        tx.executeSql(
            "create table if not exists hotels (id integer primary key not null, title text, stars text);"
        );
    });
}
...
export function AddHotelDB(title, stars) {
    db.transaction(
        (tx) => {
            tx.executeSql(
                "insert into hotels (title, stars) values (?, ?)",
                [title, stars],
                (_, {rowsAffected, insertId}) => {
                    if (rowsAffected > 0) {
                        console.log(`Новая запись в БД: ID ${insertId}; [${title}, ${stars}]`);
                    } else {
